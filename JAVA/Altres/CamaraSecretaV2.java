...
public class CamaraSecretaV2 {
    ...
    static void configurarDificultad() {
        ...
        System.out.println("Introduce la nueva longitud (min 1 - max 10): ");
        int longitud = sc.nextInt();
        do {
            if (longitud < 1 || longitud > 10) {
                System.out.println("Fuera del rango");
            }else if (longitud >= 1 || longitud <= 10) {
                System.out.println("Longitud modificada a "+longitud);
                LONGITUD_CLAVE = longitud;
            }
        } while (longitud < 1 || longitud > 10);

        System.out.println("Introduce el nuevo valor maximo (min 2 - max 10): ");
        int valorMax = sc.nextInt();
        do {
            if (valorMax < 2 || valorMax > 9) {
                System.out.println("Fuera del rango");
            }else if (valorMax >= 2 || valorMax <= 9) {
                System.out.println("Valor maximo modificado a "+valorMax);
                MAX_VALOR = valorMax;
            }
        } while (valorMax < 1 || valorMax > 10);
    ...
    static int[] pedirCombinacionUsuario() {
        ...
        // TODO: Bucle for para pedir cada dígito. 
        // OBLIGATORIO: Usar leerEntero() para pedir cada número.
        for (int i = 0; i < LONGITUD_CLAVE; i++) {
            intento[i]= leerEntero("Número en posicion "+ (i+1),1, MAX_VALOR);
        }
        
        return intento; // Cambiar este return por el valor correcto
    }
    ...
    static boolean comprobarIntento(int[] secreto, int[] intento) {
        ...
        return 0; // Cambiar este return por el valor correcto
    }
